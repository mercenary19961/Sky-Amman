...
- **Real catalogue = the DABOUQ-7 villas** (2026-06-10): the demo Dabouq/Abdoun seed was replaced by the real [`ProjectsSeeder`](database/seeders/ProjectsSeeder.php) built from `DABOUQ-7 008.pdf` — 8 villas, group "Dabouq 7", in Dabouq/Amman. Villas **3,4,5,7,8** = Ready / for-sale with land+built-up areas, 2 floors, **4 bed / 3 bath stored but HIDDEN via `hidden_specs`** (admin reveals later), completion year blank; villas **1,2,6** = sold (no areas). Every villa (incl. sold) ships a committed render gallery at `public/images/projects/dabouq-7-villa-{n}/NN.webp` (4–12 webp shots each, converted from the client's OneDrive renders). The seeder also force-removes any stale/demo projects not in the catalogue (detaching their inquiries first).
