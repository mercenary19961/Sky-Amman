...
- **Consent: CookieYes** (same vendor as the HardRock site), opt-in via `COOKIEYES_SITE_ID`, rendered **above** the GTM snippet so Google Consent Mode defaults are set before any tag fires (ordering is pinned by a test). ⚠️ The site ID is **per-domain** — mint a new one for skyamman.com rather than reusing HardRock's, or this site's consent records land in HardRock's account. **Differences from HardRock:** that project has no CSP at all, so CookieYes needed no allowlisting there; here it needs `cdn-cookieyes.com` (script) + `log.cookieyes.com` (consent log). Recommended config: **geo-target the banner to EEA/UK only** so the primary Jordan/Gulf audience isn't nagged, and enable **Consent Mode v2** (required since 2024-03 for Google Ads remarketing/measurement to work at all on EEA traffic).
...
- [ ] **Privacy policy page + consent go-live.** The footer's "Privacy policy" link is a dead `href="#"` ([Footer.tsx:341](resources/js/Components/Layout/Footer.tsx#L341)) with no page behind it, while the contact form already stores name/email/phone/`ip_address` and the newsletter stores emails — so the notice obligation (Jordan PDPL No. 24 of 2023) exists independently of the marketing tags. To close: (1) build a content-only `/privacy` page on the existing About/Security pattern (CMS-editable so the client revises copy without a deploy) + seed a `pages` row, (2) point the footer link at it, (3) create the CookieYes site for skyamman.com and set `COOKIEYES_SITE_ID`, geo-targeted to EEA/UK with Consent Mode v2 on. **Policy wording needs Jordanian counsel — scaffold the structure, don't invent legalese.**
...
> - **CookieYes consent banner wired (dormant until `COOKIEYES_SITE_ID` is set)**, ported from the HardRock site but adapted: HardRock has no CSP, so there the script "just worked"; here it needed `cdn-cookieyes.com` + `log.cookieyes.com` allowlisted. Loads **before** GTM (Consent Mode defaults), excluded from `/admin/*`, ordering pinned by a test. ⚠️ **Still needs a Sky-Amman-specific site ID** — HardRock's is per-domain and must not be reused. **Open item: the footer's "Privacy policy" link is `href="#"`** ([Footer.tsx:341](resources/js/Components/Layout/Footer.tsx#L341)) and no privacy page exists, while the contact form already collects name/email/phone/`ip_address` and the newsletter collects emails — that notice obligation predates the trackers (Jordan PDPL No. 24 of 2023). See Remaining.
