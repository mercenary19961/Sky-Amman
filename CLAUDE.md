...
> **📍 Doc sync:** CLAUDE.md last synced to commit `4f5323b` — 2026-07-05 11:10 (Sun).
...
- [~] **Custom domain DNS (2026-07-01) — DNS done, app-side switchover pending.** Domain went live as a **`www`-canonical + apex-redirect** setup (NOT Cloudflare — DNS is on the client's provider via Almond Solutions): `www.skyamman.com` → CNAME → Railway (Let's Encrypt cert issued ✅); apex `skyamman.com` → old-host `A` record with a `web.config` 301 → `https://www.skyamman.com`. **Remaining app-side steps** (main Railway service): set `APP_URL=https://www.skyamman.com` + redeploy, swap `TURNSTILE_*` to domain-bound keys minted for `www.skyamman.com`, set the `og_image_url` DB setting, and remove the stale `skyamman.com` custom-domain entry in Railway. See the rewritten "Switching to the client's custom domain" checklist.
