...
- **Still open:** the footer's "Privacy policy" link is `href="#"` and the banner links to `/privacy`, which **does not exist yet**. See Remaining.
...
- [ ] **Privacy policy page (`/privacy`) — now a broken link in TWO places.** The footer's "Privacy policy" is a dead `href="#"` ([Footer.tsx:341](resources/js/Components/Layout/Footer.tsx#L341)) **and** the consent banner links to `/privacy`, which 404s. The contact form already stores name/email/phone/`ip_address` and the newsletter stores emails, so the notice obligation (Jordan PDPL No. 24 of 2023) exists independently of the marketing tags. To close: build a content-only `/privacy` page on the existing About/Security pattern (CMS-editable so the client revises copy without a deploy) + seed a `pages` row, then point both links at it. It should disclose the cookie categories the banner offers and name the tags GTM loads. **Policy wording needs Jordanian counsel — scaffold the structure, don't invent legalese.**
