...
- [x] **Content CRUDs (homepage + about imagery)** — five admin sections that promote former seed-only/hardcoded home content to editable records (all under the **Content** sidebar group, editor-accessible, change-log-tracked where soft-deletable):
  ...
  - **Head of Departments** ([DepartmentMemberController](app/Http/Controllers/Admin/DepartmentMemberController.php), `/admin/department-members`) — bilingual name + role, photo upload, reorder, active toggle.
...
> **Last updated:** 2026-06-10 — **Real Dabouq catalogue + Content CRUDs + public/contact polish.** Homepage content that was seed-only or hardcoded is now editor-managed, and the demo projects were replaced with the real client brochures.
