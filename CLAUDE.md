...
- **⚠️ CSP is the silent-failure trap — see the gotcha below.** Adding any NEW tag in the GTM UI (Meta Pixel, LinkedIn Insight, Hotjar…) requires adding its host to [SecurityHeaders](app/Http/Middleware/SecurityHeaders.php). GTM's "no developer needed" promise is only partly true behind a strict CSP; tell the marketing team this up front.
...
**Lesson / cross-project:** for ANY third-party tag, allowlist three things, not one: where the script loads from (`script-src`), where it sends data (`connect-src`), and any iframe it injects (`frame-src`). If Google Signals / Ads remarketing gets enabled later, `stats.g.doubleclick.net` needs adding too. This applies to Meta Pixel, Hotjar, LinkedIn Insight and friends identically. **Diagnostic tell:** tags report as fired but the analytics property shows no traffic → open DevTools console and look for `Refused to connect` CSP violations, not for JS errors.
